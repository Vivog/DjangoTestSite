...
class ProjectsList(ListView, PortalMixin):
    ...
    queryset = Projects.objects.prefetch_related('author', 'category', 'docs', 'pics', 'reviewsprojects_set')
    paginate_by = 2
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        context['projs'] = Projects.objects.only('name', 'slug', 'pics')
...
def download_doc(request, id):
    ...
    path = str(Projects.objects.get(pk=id).docs)
...
class ProjectsCatsDetail(DetailView, PortalMixin):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        object_list = Projects.objects.prefetch_related('category', 'author', 'docs', 'pics')
...
class AddReviewProject(View):
    ...
    def post(self, request, slug):
        ...
        project = Projects.objects.prefetch_related('category', 'author', 'docs', 'pics').get(slug=slug)
        if form.is_valid():
